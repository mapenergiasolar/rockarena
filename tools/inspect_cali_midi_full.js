...
function parseMidi(filePath) {
            ...
            if (status === 0xFF) {
                ...
                if (eventType === 0x90 || eventType === 0x80) {
                    ...
                    if (eventType === 0x90 && velocity > 0) {
                        const noteTime = ticksToSeconds(trackTick);
                        if (noteTime > maxTimeOverall) maxTimeOverall = noteTime;
                        if (trackName.toLowerCase().includes('lead')) {
                            leadNotesTimes.push(noteTime);
                        }
                    }
    ...
    console.log("MIDI Full Duration:", maxTimeOverall.toFixed(2), "seconds");
    if (leadNotesTimes.length > 0) {
        leadNotesTimes.sort((a, b) => a - b);
        console.log("Lead Guitar Note Count:", leadNotesTimes.length);
        console.log("Lead Guitar First Note:", leadNotesTimes[0].toFixed(3), "seconds");
        console.log("Lead Guitar Last Note:", leadNotesTimes[leadNotesTimes.length - 1].toFixed(3), "seconds");
    } else {
        console.log("No lead guitar notes found.");
    }
}
